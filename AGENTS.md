...
- **ART subprocess shutdown uses defense-in-depth: conditional `os._exit()` + parent-side join timeout.** vLLM 0.21+ non-daemon threads prevent `asyncio.run()` from completing shutdown, so the subprocess needs `os._exit()` to prevent hangs — but it must be **conditional** (`if result is not None`), otherwise it kills multi-iteration training after iteration 1 before the loop can continue. The parent-side `proc.join(timeout=...)` (default 2h, configurable via `TRAINING_HUB_JOIN_TIMEOUT`) is the safety net: if the subprocess is still alive after the timeout, the parent terminates it. This two-layer approach lets multi-iteration training complete normally while still preventing indefinite hangs on the error path.
- **ART 0.5.18 removed `model.train()` — use `hasattr` for API compatibility.** `TrainableModel.train()` was removed in art 0.5.18; the replacement is `backend.train(model, trajectory_groups, learning_rate=...)`. The current code uses a `hasattr(model, "train")` check to support both art 0.5.17 and 0.5.18. When ART makes further API changes, follow the same pattern: feature-detect with `hasattr` rather than pinning old versions.
...
- **Unconditional `os._exit()` broke multi-iteration GRPO:** The hang fix (PRs #132/#133) made `os._exit(0)` unconditional in the subprocess `finally` block. This killed the process after iteration 1 completed, before the training loop could start iteration 2. Simultaneously, ART 0.5.18 removed the `TrainableModel.train()` method, causing `AttributeError` on the now-dead path. Fix: (1) revert `os._exit()` to conditional (`if result is not None`) — the parent-side `proc.join(timeout=...)` is the hang backstop, (2) add `hasattr` check to use `backend.train()` for art 0.5.18+. Lesson: `os._exit()` in a training loop is inherently dangerous for multi-iteration — the subprocess-side force-exit should only fire after the training loop has finished, not after each iteration. Use the parent-side join timeout as the primary hang defense.
...
# Validated on 1x H100, multi-iteration GRPO (ART backend, AIPCC 3.5 GA stack)
num_iterations=3, group_size=2, prompt_batch_size=5
lora_r=8, max_tokens=64, gpu_memory_utilization=0.3
Result: all 3 iterations completed, rewards [0.281, 0.188, 0.406]
Training time: ~198s total
Environment: openpipe-art==0.5.18, vllm==0.21.0+rhaiv.10, torch==2.11.0, trl==1.8.0
```

...
# Install with GRPO support (ART backend only)
pip install -e .[grpo,lora]

# Install with GRPO + verl backend (verl is not on the AIPCC index)
pip install -e .[grpo,grpo-verl,lora]
...
```bash
pip install -e .[grpo,lora]           # GRPO backend (ART only)
pip install -e .[cuda] --no-build-isolation  # CUDA kernels (after grpo)
```
Add `grpo-verl` to the first line if you need the verl backend (`pip install -e .[grpo,grpo-verl,lora]`).
Note: verl is not on the AIPCC index — only install it in non-AIPCC environments.
...
- **trl 1.x renamed `tokenizer` to `processing_class` in `SFTTrainer`.** This follows the same rename as `transformers.Trainer`. Code that passes `tokenizer=...` to `SFTTrainer` will fail or behave unexpectedly with trl 1.x. Use `processing_class=...` instead (backward-compatible with older versions that accept both).
...
- **ART subprocess hang prevention uses two layers of defense.** vLLM V1's EngineCore and APIServer spawn non-daemon threads that prevent `asyncio.run()` from completing shutdown. The hang occurs during `asyncio.run()`'s internal teardown — AFTER the coroutine's `finally` block completes but BEFORE `asyncio.run()` returns — so `os._exit()` in the async `finally` only helps on the success path. The current fix: (1) `os._exit(0)` in the async `finally` block, but ONLY on success (`if result is not None`) — unconditional `os._exit` kills multi-iteration training; (2) parent-side `proc.join(timeout=N)` with `TRAINING_HUB_JOIN_TIMEOUT` env var (default 7200s) as defense-in-depth — if the subprocess hasn't exited after the timeout, it is terminated. Results are saved to disk before shutdown, so the parent reads them via `training_results.json`/`.training_error` files.
- **ART 0.5.18 moved `train()` from `TrainableModel` to `LocalBackend`.** Code that calls `model.train(train_groups, config=TrainConfig(...))` must switch to `backend.train(model, train_groups, learning_rate=...)`. Use `hasattr(model, "train")` to support both ART 0.5.17 and 0.5.18.
- **ART 0.5.18 requires megatron import stubs when `megatron-core` is unavailable.** ART 0.5.18 added Qwen3.5 Megatron support with unconditional module-level imports from `megatron.core` and `megatron.bridge`. These fire even on the non-Megatron Unsloth backend path. When `megatron-core` is not on the package index (e.g., AIPCC), import of ART fails. The fix is a `sys.meta_path` finder that auto-creates stub modules for any `megatron.*` import. The stub uses a metaclass where `__call__` returns `cls` (not an instance) so it works as both a decorator and a base class. This stub should be removed when ART makes the megatron import lazy.
...
- **`os._exit` fix broke multi-iteration GRPO (3 PRs in 36 hours):** The initial subprocess hang fix (PR #132) made `os._exit(0)` unconditional in the async `finally` block. This killed the subprocess after every iteration, breaking multi-iteration training. PR #133 added parent-side `proc.join(timeout)` as defense-in-depth, revealing that the hang occurs during `asyncio.run()`'s internal shutdown — AFTER the coroutine's finally block but BEFORE `asyncio.run()` returns — so `os._exit` in the async finally doesn't always reach. PR #135 reverted to conditional `os._exit` (success path only), since the parent-side timeout handles the failure case. Also fixed `model.train()` being removed in ART 0.5.18 (`hasattr` shim for `backend.train()`). Lesson: subprocess shutdown fixes must be tested with multi-iteration training, not just single-iteration; and async `finally` blocks can be preempted by `asyncio.run()`'s own teardown.
- **Megatron stub required 3 iterations for ART 0.5.18 compatibility:** PR #118 only stubbed `megatron.bridge.*` submodules; ART also imports from `megatron.core.models` and `megatron.core.ssm`. PR #127 added a `sys.meta_path` finder for all `megatron.*` imports, but `_StubMeta.__call__` created instances (not callable as decorators). PR #130 fixed `__call__` to return `cls`. Lesson: when stubbing imports for a third-party library, test with the library's full import graph, not just the imports you found by reading its source — decorators and base classes require the stub to be callable and re-callable.
- **OSFT state dict dtype reverted by meta-device initialization (mini_trainer PR #114):** In the distributed OSFT loading path, rank 0 loads the model with `torch_dtype=bfloat16`, but the model is re-created on meta device where `nn.Embedding` defaults to float32. The dtype registry captured float32 from meta device, and `post_fsdp2_wrap_synchronize_state_dict_across_procs` converted the bfloat16 state dict values back to float32 to match — breaking FSDP2 training. Fix: remove the dtype conversion entirely; the rank-0 state dict is already in the correct dtype. Lesson: meta-device initialization does not respect `torch_dtype` for all layer types — do not use meta-device dtypes as ground truth.
...
- **Do not use meta-device dtypes to "correct" rank-0 state dict values.** `nn.Embedding` defaults to float32 on meta device regardless of `config.torch_dtype`. If you capture dtypes from a meta-device model and use them to convert state dict values from rank 0 (which loads with the actual training dtype, e.g., bfloat16), you silently revert bfloat16 parameters back to float32. This breaks FSDP2 training. The state dict from rank 0 is already in the correct training dtype (mini_trainer PR #114).

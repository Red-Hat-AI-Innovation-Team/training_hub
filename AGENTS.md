...
- **transformers v4 and v5 have breaking API differences.** v5 renamed `additional_special_tokens` to `extra_special_tokens` in the tokenizer API. Code that supports both versions needs version-conditional handling (e.g., a `SPECIAL_TOKENS_KEY` constant that resolves based on installed version). This is a cross-repo concern: instructlab/training may pin transformers v5 while training_hub's LoRA backend (via unsloth) requires transformers v4. When adding tokenizer or special token handling, check which transformers version each backend environment uses.
- **trl 0.24.0 breaks with transformers v5:** `_is_package_available()` returns `(bool, version)` tuples instead of plain bools. Code like `if _is_package_available("pkg"):` evaluates `bool((False, None))` as `True`, importing packages that aren't installed. This causes bizarre import errors. If you see unexpected `ImportError`s after upgrading trl or transformers, check whether `_is_package_available` return values are being used correctly.
...
- **`isinstance(True, int)` returns `True` in Python.** Boolean inputs bypass naive `isinstance(x, int)` validation checks. When validating user inputs for numeric parameters (e.g., `num_gpus`, `batch_size`), check `isinstance(x, bool)` first and reject it before the int check. This is especially important in templated notebooks and convenience functions where user input flows directly into training configs.
- **Gemma 3 and similar VLM-registered models need special handling for SFT.** `AutoModelForCausalLM.from_pretrained()` does NOT guarantee a text-only model. Gemma 3, Gemma 3n, Ministral, and Mistral3-VLM are registered in both `MODEL_FOR_CAUSAL_LM_MAPPING` and `MODEL_FOR_IMAGE_TEXT_TO_TEXT_MAPPING` in transformers. When loaded via `AutoModelForCausalLM`, they resolve to the full VLM class (e.g., `Gemma3ForConditionalGeneration`), which crashes during FSDP-wrapped SFT because the text-only training loop can't handle the vision tower. Check for class names containing `ForConditionalGeneration` on dual-registered configs and route through VLM backbone extraction.
...
- **Check for existing open PRs before creating a new one.** Run `gh pr list --state open` and check if a PR already exists for the same issue or feature. Creating duplicate PRs wastes compute, reviewer time, and creates confusion. This was the single most wasteful pattern observed in agent work — the same GitHub issue was worked 9+ times, creating 9+ duplicate PRs.
- **Validators must SSH to GPU nodes for GPU testing — never claim tests pass without hardware.** If your environment has no GPU, SSH to the designated GPU node and run tests there. Do not skip GPU testing or claim "GPU validation not available" when GPU nodes are accessible. End-to-end testing on hardware is required before approving PRs that touch training code paths.
- **Never include internal infrastructure details in public GitHub comments.** Do not post IP addresses, ports, hostnames, or connection details in PR descriptions, comments, or commit messages on public repositories. You may describe *what* infrastructure was used (e.g., "tested on A100 node") but never *how* to access it.
...
- **Use `enforce_eager=True` on NVIDIA driver 580+/610+ to bypass vLLM torch.fx crashes.** On these newer drivers, vLLM's torch.fx compilation pipeline crashes with `RuntimeError: Tried to erase Node size_X but it still had N users in the graph` (affects vLLM 0.19-0.23 with torch 2.10-2.11). Setting `enforce_eager=True` disables torch.compile and CUDA graph capture in vLLM, bypassing the issue. The parameter is available on `lora_grpo()`, `grpo()`, and `LoRAGRPOAlgorithm.train()`.
- **Unsloth must be imported before ART in the spawned subprocess.** The import chain `art.unsloth.service` → `trl.GRPOTrainer` → `vllm.GuidedDecodingParams` fails on vLLM ≤0.21 because `GuidedDecodingParams` doesn't exist yet. Unsloth patches `vllm.sampling_params` with a shim, but only if imported first. The fix is `import unsloth` before `import art` in `_subprocess_entry`. If you see `ImportError: cannot import name 'GuidedDecodingParams' from 'vllm.sampling_params'`, check import order.
...
- **NVIDIA driver 580+/610+ broke vLLM torch.fx compilation (PR #98):** vLLM crashed with `RuntimeError: Tried to erase Node size_X but it still had N users in the graph` during torch.fx compilation. Only happened on nodes with NVIDIA drivers 580+ or 610+. Affected vLLM 0.19-0.23 with torch 2.10-2.11. Fix: pass `enforce_eager=True` to disable torch.compile and CUDA graph capture. Lesson: driver version can affect torch.fx/torch.compile behavior — when a crash only happens on specific nodes, check driver version before blaming the library.
- **Unsloth import order in ART subprocess (PR #98):** `ImportError: cannot import name 'GuidedDecodingParams' from 'vllm.sampling_params'` in the ART spawned subprocess. Root cause: trl's `GRPOTrainer` imports `GuidedDecodingParams` from vLLM, but this class doesn't exist in vLLM ≤0.21. Unsloth monkey-patches `vllm.sampling_params` with a shim, but only if imported first. The subprocess imported `art` (which imports `trl`) before `unsloth`, hitting the missing class. Fix: add `import unsloth` before `import art` in `_subprocess_entry`. Lesson: in spawned subprocesses, import order matters differently than in the parent — the subprocess starts with a clean module state and needs all patches applied before the libraries that depend on them.
- **transformers v4/v5 `additional_special_tokens` rename (instructlab/training PR #684):** instructlab/training pinned `transformers>=5.0.0`, but v5 renamed `additional_special_tokens` to `extra_special_tokens`. The LoRA training path (via unsloth) requires transformers v4.57, which uses the old name. Fix: version-conditional handling with `SPECIAL_TOKENS_KEY` constant and `get_extra_special_tokens()` helper. Lesson: different backends may pin different major versions of shared dependencies — always check cross-backend compatibility when upgrading.

...
- **Test notebooks with `jupyter execute` on target hardware before merging.** Default configs must work out-of-the-box — e.g., a notebook defaulting to `num_gpus=1` with an 8B model will fail on a single A100 if the algorithm requires more memory than available. Always validate that the notebook runs end-to-end with its shipped defaults on the target GPU class.

...
- **Use `LOCAL_RANK` (not global rank) for console log gating in multi-node training.** Gating on `dist.get_rank() == 0` means only node 0's rank 0 prints progress — worker nodes have zero visibility. Use `int(os.environ.get("LOCAL_RANK", 0)) == 0` so each node's local rank 0 prints training progress (loss, lr, tokens/sec, tqdm). Keep wandb/mlflow logging gated on global rank 0 to avoid duplicate metric entries.
...
- **GRPO `training_metrics.jsonl` must include `max_steps` for Kubeflow progress.** The Kubeflow SDK metrics handler uses `max_steps` to calculate `progressPercentage`. Set it to `num_iterations` in both rollout and train phase metric entries. Without it, the SDK reports progress as `None`.
...
- **ART's async cleanup errors should not overwrite successful results.** After training completes, vLLM shutdown can trigger `ConnectionRefusedError` in ART's background health-check task. If the subprocess writes this to `error_path`, Kubeflow restarts the pod even though training succeeded. The fix (PR #78) checks whether `results_path` exists before treating cleanup exceptions as failures — if training completed, the error is logged as a warning instead. Do not revert this behavior.
...
- **ART post-training cleanup `ConnectionRefusedError` caused Kubeflow pod restart.** After vLLM shuts down at end of training, ART's background health-check task throws `ConnectionRefusedError`. The subprocess wrote this to `error_path`, which made Kubeflow think training failed and restarted the pod — even though results were already on disk. Fix: check for `results_path` before writing to `error_path`; if training completed, log the cleanup error as a warning. Lesson: post-training cleanup errors in a subprocess pipeline should not overwrite success signals.
- **AdamW silently corrupts OSFT orthogonal constraints.** AdamW's element-wise moment rescaling (`m̂_t / (√v̂_t + ε)`) destroys the orthogonal structure of projected gradients, causing parameter updates to leak into the frozen subspace over time. The corruption is gradual and doesn't cause obvious errors — it manifests as subtle quality degradation. Fix: add post-step parameter re-projection to guarantee `U_low` and `V_low` never drift into the frozen subspace. Lesson: standard optimizers are not safe to use with orthogonal projection constraints without re-projection — the optimizer's internal state transformations can violate the projection invariants.
- **Multi-node training progress invisible on worker nodes.** Console log gating used `dist.get_rank() == 0` (global rank), so only node 0, GPU 0 printed progress. Worker nodes had zero visibility into training — operators on those nodes couldn't tell if training was running or stuck. Fix: gate on `LOCAL_RANK == 0` so each node's rank 0 prints progress. Lesson: distinguish between per-node visibility (LOCAL_RANK) and global singleton operations (wandb logging uses global rank 0).

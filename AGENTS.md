...
- **`datasets` and `pyarrow` must be version-compatible.** `pyarrow>=21.0.0` removed `PyExtensionType`, which `datasets<5.0.0` still references. Standalone mini_trainer installs can resolve to incompatible versions that training_hub's dependency tree otherwise masks. Pin `datasets>=5.0.0` when using `pyarrow>=21`. If you see `AttributeError: module 'pyarrow' has no attribute 'PyExtensionType'` during test collection, this is the cause.
...
- **Import unsloth before art in the ART subprocess.** trl 1.x imports `GuidedDecodingParams` from vLLM, but that class doesn't exist in vLLM ≤0.21. Unsloth patches `vllm.sampling_params` with a shim, but only when imported first. In `_subprocess_entry`, `import unsloth` must come before `import art`. Without this ordering, ART subprocess startup crashes with `ImportError`.
- **Use `enforce_eager=True` on NVIDIA driver 580+/610+.** vLLM's `torch.fx` compilation pipeline crashes with `RuntimeError: Tried to erase Node size_X but it still had N users in the graph` on newer drivers (affects vLLM 0.19-0.23 with torch 2.10-2.11). Setting `enforce_eager=True` disables `torch.compile` and CUDA graph capture, bypassing the issue. Exposed via `lora_grpo()`, `grpo()`, and `LoRAGRPOAlgorithm.train()`.
- **LoRA SFT `packing` must be in `SFTConfig`, not `SFTTrainer` kwargs.** In trl 1.x, Unsloth auto-enables `padding_free=True` on `SFTConfig`. If `packing` is only passed as a `SFTTrainer` constructor kwarg, trl's validation sees `padding_free=True` without packing and raises `ValueError`. Move `packing=True` and `packing_strategy="bfd"` into the `SFTConfig` constructor — this is backward-compatible with trl 0.24.
- **`assistant_only_loss` must be `False` for passthrough datasets.** When `dataset_type="passthrough"`, the data is raw text, not conversational. Setting `assistant_only_loss=True` (the default for message-based datasets) causes trl to reject the combination. Gate `assistant_only_loss` on the actual dataset format.
- **Both SFT and OSFT backends default to `beta2=0.95` (non-standard).** PyTorch's AdamW default is `beta2=0.999`. Both `instructlab.training.TrainingArgs` and `mini_trainer.TrainingArgs` override this to `0.95`. Be aware of this when comparing training results against reference runs from other frameworks, or when tuning optimizer hyperparameters.
- **Callback classes must be self-contained for torchrun serialization.** Both mini_trainer and instructlab-training serialize callbacks via `inspect.getsource()` + base64 encoding to cross the torchrun subprocess boundary. Callbacks that reference closures, module-level variables, or non-importable state will fail to deserialize. Define all callback logic within the class body.
...
- **`datasets`/`pyarrow` version mismatch broke standalone mini_trainer installs:** Without a floor on `datasets`, the resolver could install `datasets==2.14.4` alongside `pyarrow==24.0.0`. `pyarrow>=21` removed `PyExtensionType`, which `datasets<5.0.0` still uses. This crashed all test collection with `AttributeError`. Training Hub's own dependency tree pulled `datasets>=5.0.0` transitively, masking the issue — it only appeared in standalone mini_trainer installs. Fix: pin `datasets>=5.0.0` in mini_trainer's `pyproject.toml`. Lesson: transitive dependency resolution can mask version conflicts that appear in standalone installs — test install paths independently.
- **trl 1.x `padding_free` crash when packing was set as SFTTrainer kwarg:** In trl 1.x, Unsloth auto-enables `padding_free=True` on `SFTConfig`. When `packing=True` was passed only as a `SFTTrainer` constructor kwarg (not in `SFTConfig`), trl's validation saw `padding_free=True` without packing enabled and raised `ValueError`. The fix was moving `packing` and `packing_strategy="bfd"` into the `SFTConfig` constructor. This is backward-compatible with trl 0.24. Lesson: when a library restructures which config object owns which parameter, kwarg placement must move with it.
- **vLLM `torch.fx` crash on NVIDIA driver 580+/610+:** vLLM's compilation pipeline failed with `RuntimeError: Tried to erase Node size_X but it still had N users in the graph`. This affected vLLM 0.19-0.23 with torch 2.10-2.11 on newer NVIDIA drivers. Fix: pass `enforce_eager=True` to disable `torch.compile` and CUDA graphs. Lesson: driver upgrades can break JIT compilation pipelines even when CUDA toolkit versions haven't changed — `enforce_eager` is a reliable workaround.
- **trl 1.x `GuidedDecodingParams` import failure in ART subprocess:** trl 1.x imports `GuidedDecodingParams` from vLLM at top level, but the class doesn't exist in vLLM ≤0.21. Unsloth patches `vllm.sampling_params` with a shim, but only when imported first. The ART subprocess imported `art` before `unsloth`, so the patch wasn't applied. Fix: add `import unsloth` before `import art` in `_subprocess_entry`. Lesson: import order matters when one library monkey-patches another — document and enforce the ordering.

...
- **trl 1.x moved packing config into SFTConfig.** In trl >= 1.0, Unsloth auto-enables `padding_free=True` on SFTConfig. `packing` and `packing_strategy` must be set on SFTConfig, not as SFTTrainer constructor kwargs — otherwise trl's validation raises ValueError. Same applies to `assistant_only_loss`: it was incorrectly hardcoded to `True` for passthrough datasets when it should be `False`. See training_hub PR #104.
- **Unsloth must be imported before ART in the subprocess.** The ART backend subprocess must import unsloth before art to ensure the `GuidedDecodingParams` shim is applied before trl imports it. Without this ordering, the import fails. See training_hub PR #98.
- **Both SFT/OSFT backends use a non-standard AdamW `beta2=0.95` default** (PyTorch's default is 0.999). This was an intentional choice for training stability. When comparing training runs with external baselines or debugging convergence, verify the beta2 value matches expectations.
- **mini_trainer supports a class-based callback system.** Callbacks must be self-contained (all imports inside the function body) because they are serialized via `inspect.getsource` + base64 to cross the torchrun subprocess boundary. Callbacks run asynchronously on rank-0 only; exceptions do not propagate to the training loop. See mini_trainer PR #109.
- **`enforce_eager=True` bypasses torch.fx compilation crashes on newer NVIDIA drivers.** NVIDIA drivers 580+/610+ cause `torch.fx` compilation to crash in vLLM 0.19-0.23 with torch 2.10-2.11. The parameter is exposed on `lora_grpo()`, `grpo()`, and `LoRAGRPOAlgorithm.train()`. See training_hub PR #98.
- **Some models are dual-registered in transformers' model mappings.** Models like Gemma 3 appear in both `MODEL_FOR_CAUSAL_LM_MAPPING` and `MODEL_FOR_IMAGE_TEXT_TO_TEXT_MAPPING`. `AutoModelForCausalLM` may resolve to the full VLM class rather than a text-only class. If a model unexpectedly loads vision components during text-only training, check whether its config class appears in both mappings. See instructlab/training PR #695.
- **`trust_remote_code=True` must be set on ALL AutoConfig/AutoTokenizer/AutoModel calls.** When supporting models with custom code (Nemotron, Ministral, Qwen3.5), ensure the flag is set consistently across the entire codebase — not just the main model loading path. Missing it on data processing or tokenizer utility calls causes silent failures. See instructlab/training PR #696.
...
- **datasets 5.0.0 pyarrow compatibility break:** After `datasets` 5.0.0 was released (June 2026), existing CI broke due to pyarrow incompatibility with older datasets versions. Fix: pin `datasets>=5.0.0` (mini_trainer PR #110). Lesson: when CI breaks after a dependency release, check whether pinning to the new version (not the old one) resolves the compatibility issue. Also: reviewer agents with stale local pip indexes can give false CHANGES_REQUESTED reviews — always verify package existence on PyPI directly.
- **LoRA SFT packing crash with trl 1.x:** `packing` and `packing_strategy` were passed as SFTTrainer constructor kwargs. In trl >= 1.0, Unsloth sets `padding_free=True` on SFTConfig, and trl's validation rejects packing as a constructor kwarg when padding_free is enabled. Fix: move packing params into SFTConfig (training_hub PR #104). Same PR also fixed `assistant_only_loss` being hardcoded to `True` for passthrough datasets when it should be `False`. Lesson: when a library major-versions (trl 0.x to 1.x), re-read the constructor signature and config class — parameters may have moved.
- **vLLM torch.fx crash on NVIDIA driver 580+/610+:** vLLM 0.19-0.23 with torch 2.10-2.11 crashes during torch.fx compilation on newer NVIDIA drivers. Fix: pass `enforce_eager=True` to bypass torch.fx compilation (training_hub PR #98). Lesson: driver upgrades can break JIT compilation paths — expose eagerness flags rather than pinning driver versions.
- **Unsloth import ordering in ART subprocess:** The ART backend subprocess must import unsloth before art to ensure the `GuidedDecodingParams` shim is applied before trl imports it. Without this ordering, the import fails with missing attributes. Fix: reorder imports in subprocess entry point (training_hub PR #98). Lesson: import ordering matters when libraries monkey-patch shared dependencies.
- **transformers + kernels incompatibility:** `transformers==5.9.0` is incompatible with `kernels==0.15.1`, breaking smoke tests across all models. Workaround: `pip uninstall kernels`. Lesson: the `kernels` package (used for optimized attention kernels) may conflict with newer transformers versions — if import errors mention kernels, try uninstalling it first.
- **Duplicate PR flood from agent dispatch:** The dev agent created 9 near-identical PRs for the same GitHub issue (#34) because each Multica issue dispatch triggered a new PR without checking for existing PRs. Fix: agents must run `gh pr list --state open --search '<issue-keywords>'` before creating PRs, and update the existing branch if one exists. Lesson: when agent workflows dispatch the same task multiple times, the agent must dedup by checking existing open PRs.
- **Agent GitHub token missing PR review and commit status permissions:** Agents could push code and create PRs but could not post PR reviews or set commit statuses on GitHub. This meant branch protection status checks could never be satisfied by the agent pipeline. Workaround: agents posted reviews as Multica issue comments instead. Lesson: verify that agent tokens have `pull_requests:write` and `statuses:write` scopes before deploying agent review workflows.

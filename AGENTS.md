...
│   ├── gepa.py              # GEPA prompt optimization algorithm
│   ├── its_rollout.py       # ITS Hub rollout adapter for GRPO
...
- **ART subprocess requires `os._exit()` to prevent vLLM 0.21+ hang.** vLLM V1's EngineCore and APIServer spawn non-daemon threads that prevent `asyncio.run()` from completing its shutdown. Without `os._exit()`, `proc.join()` in the parent hangs indefinitely after training completes. The fix uses `os._exit(0)` on success (in `_run_training`'s finally block) and `os._exit(1)` on failure (in `_subprocess_entry`'s except block), with an `exit_code` variable to track which path was taken. Results are saved to disk before exit, so the parent reads them via `training_results.json`/`.training_error` files after `proc.join()` returns.
...
- **Import unsloth before art in ART subprocess.** ART's module-level `from trl import GRPOTrainer` tries to import `GuidedDecodingParams` from vLLM, which doesn't exist in vLLM ≤0.21. Unsloth patches `vllm.sampling_params` with a shim, but only if imported first. Always add `import unsloth` before `import art` in `_subprocess_entry`.
- **LoRA SFT packing config must be in `SFTConfig`, not `SFTTrainer` kwargs, for trl 1.x.** In trl 1.x, Unsloth auto-enables `padding_free=True` on `SFTConfig`. If `packing` is passed as a `SFTTrainer` kwarg (not in `SFTConfig`), trl raises `ValueError` because it sees `padding_free=True` without packing enabled. Move `packing` and `packing_strategy="bfd"` into `SFTConfig` construction. This is backward-compatible with trl 0.24.
- **`assistant_only_loss` must be `False` for passthrough datasets.** When `dataset_type="passthrough"`, the dataset uses `dataset_text_field="text"` (plain text, not conversational). Setting `assistant_only_loss=True` on non-conversational data silently degrades training quality. Gate the flag on dataset type.
- **Both SFT backends default to AdamW `beta2=0.95` (not PyTorch's 0.999).** Both `instructlab-training` and `mini-trainer` use `beta2=0.95` by default. This can surprise users comparing training runs against standard AdamW implementations or academic baselines. The parameter is exposed as an optional kwarg.
- **Callback classes must be self-contained for torchrun serialization.** The callback system serializes user-defined `TrainerCallback` subclasses across the torchrun subprocess boundary via `inspect.getsource()` + base64 encoding. All imports must be inside the class body, not at module scope. Closures and lambdas will fail. This applies to both mini_trainer and instructlab-training backends.
- **SFT convenience function derives optional param defaults.** `data_output_dir` defaults to `ckpt_output_dir/data` and `max_batch_len` defaults to `max_seq_len` when not provided. When a param is required by `TrainingArgs` but optional from the user's perspective, derive it in `execute_training()` before constructing `TrainingArgs`, and list it in `get_optional_params()` instead of `get_required_params()`.
...
- **vLLM 0.21+ subprocess hang after successful training:** With vLLM 0.21+, the lora_grpo training subprocess hung indefinitely after training completed. The V1 engine's EngineCore and APIServer spawn non-daemon threads that prevent `asyncio.run()` from completing shutdown, blocking the parent at `proc.join()`. This was a progression from the `ConnectionRefusedError` fix (#78) — the failure mode changed from an exception to a silent hang. Fix: add `os._exit(0)` after results are saved and backend shutdown is attempted, with `os._exit(1)` on the error path so `proc.exitcode` correctly reflects failure. Lesson: vLLM's shutdown behavior changes across versions — when upgrading vLLM, verify that training subprocesses still exit cleanly after completion.
- **vLLM torch.fx crash on NVIDIA driver 580+/610+:** On nodes with newer NVIDIA drivers, vLLM's torch.fx compilation crashed with `RuntimeError: Tried to erase Node size_X but it still had N users in the graph`. Affects vLLM 0.19-0.23 with torch 2.10-2.11. Fix: pass `enforce_eager=True` to disable torch.compile and CUDA graph capture. Lesson: newer NVIDIA drivers can break vLLM's JIT compilation — if vLLM crashes during model loading on a node with recent drivers, try `enforce_eager=True` before investigating further.
- **trl 1.x `padding_free` crash when packing passed as `SFTTrainer` kwarg:** In trl 1.x, Unsloth auto-enables `padding_free=True` on `SFTConfig`. When `packing` was set as a `SFTTrainer` constructor kwarg (not in `SFTConfig`), trl's validation saw `padding_free=True` without packing enabled and raised `ValueError`. Reported by a user on the amortized training container (trl 1.5.1). Fix: move `packing` and `packing_strategy` into `SFTConfig`. Lesson: when a third-party library changes how it validates config, constructor kwargs may need to move into the config object.
- **`assistant_only_loss=True` silently degraded passthrough datasets:** The `assistant_only_loss` flag was hardcoded to `True` for all dataset types, including passthrough datasets that use `dataset_text_field="text"` (plain text, not conversational). trl's `assistant_only_loss` masking requires conversational format — applying it to plain text silently degraded training quality. Fix: set `assistant_only_loss=False` when `dataset_type="passthrough"`. Lesson: loss masking flags must be gated on the actual data format, not set unconditionally.
- **trl `GuidedDecodingParams` import failure in ART subprocess:** ART's module-level `from trl import GRPOTrainer` transitively imports `GuidedDecodingParams` from vLLM, which doesn't exist in vLLM ≤0.21. Without Unsloth's shim (which patches `vllm.sampling_params`), the import fails with `ImportError`. Fix: ensure `import unsloth` appears before `import art` in `_subprocess_entry`. Lesson: import ordering matters in subprocesses — shim patches must be applied before the library that depends on them.
- **`datasets`/`pyarrow` version mismatch broke standalone mini_trainer install:** When mini_trainer was installed standalone (not as a training_hub dependency), the resolver picked `datasets==2.14.4` alongside `pyarrow==24.0.0`. `datasets` 2.x uses `pyarrow.PyExtensionType` which was removed in pyarrow 21, causing `AttributeError` at test collection time. In training_hub's dependency graph, `datasets>=5.0.0` was resolved transitively — masking the issue. Fix: pin `datasets>=5.0.0` in mini_trainer. Lesson: transitive dependency resolution can mask version conflicts — always test standalone install in a fresh venv.

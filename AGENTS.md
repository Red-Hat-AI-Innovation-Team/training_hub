...
- **Megatron stubs use a dynamic `sys.meta_path` finder — not a hardcoded list.** ART's megatron imports expand across versions (0.5.18 added `megatron.core.models`, `megatron.core.ssm`, decorator/base-class usage). An initial enumerated list of `megatron.bridge.*` submodules broke when ART added more imports. The current approach installs `_MegatronStubFinder` on `sys.meta_path` to auto-create stubs for any `megatron.*` import on demand. Always try `__import__(pkg)` first to avoid shadowing a real `megatron-core` installation. Track `_used_stub` to decide whether the finder is needed.
- **trl 1.8+ rejects empty `reward_funcs` — monkey-patch GRPOTrainer for ART compatibility.** ART passes `reward_funcs=[]` to `GRPOTrainer`, which trl 1.8+ validates and rejects (`ValueError`). The fix subclasses `GRPOTrainer` to insert a no-op reward function when `reward_funcs` is empty. Handle both positional and keyword arg passing to prevent `TypeError: got multiple values for argument`. ART overrides `compute_loss` entirely, so the no-op is never called at runtime.
...
- **Enumerated megatron stub list broke on ART 0.5.18 upgrade:** PR #118 added a hardcoded list of `megatron.bridge.*` submodule stubs, which worked for ART 0.5.18's initial import set. But ART 0.5.18 also imports from `megatron.core.models`, `megatron.core.ssm`, and uses megatron types as decorators and base classes — all outside the enumerated list. The stub `_Stub` class also returned the class itself from `__call__` instead of a fresh instance, making `_Stub.__init__` dead code and causing cross-call attribute pollution. PR #127 replaced the enumerated list with a `sys.meta_path` finder, fixed `_Stub` instance creation via `super().__call__()`, and added debug logging. Lesson: when stubbing a third-party namespace, prefer a dynamic meta-path finder over a hardcoded list — the import surface grows with each dependency version.
- **trl 1.8+ rejected ART's empty `reward_funcs=[]`:** ART 0.5.18 passes `reward_funcs=[]` to `GRPOTrainer.__init__()`. trl 1.8+ added validation that rejects empty lists with `ValueError`. Fix: monkey-patch `GRPOTrainer` to insert a no-op reward function when `reward_funcs` is empty. The initial fix only checked `kwargs.get("reward_funcs")`, missing the case where `reward_funcs` is passed positionally (2nd arg after `model`). Code review caught this: if ART passes it positionally, the fix would set it in kwargs too → `TypeError: got multiple values`. Fix: handle both positional and keyword paths. Lesson: when monkey-patching third-party `__init__` methods, handle arguments passed both positionally and as keywords — check `args[N]` and `kwargs.get("name")` separately.
...
# Validated AIPCC 3.5 GA stack (ART backend), 8x A100-SXM4-80GB
# LoRA GRPO with Toucan tool-call verification
openpipe-art==0.5.18, vllm==0.21.0+rhaiv.10, trl==1.8.0
transformers==5.13.0, kernels==0.16.0
unsloth==2026.6.9+rhaiv.2 (ODH fork, --no-deps), unsloth-zoo==2026.6.7+rhaiv.1
torch==2.11.0
Requires: megatron stub (auto-installed), no-op reward_funcs patch (auto-applied)
Confirmed working models (LoRA): Nemotron-Nano-9B-v2, Qwen3-4B, Qwen2.5-1.5B, granite-3.1-8b, Mistral-7B
```

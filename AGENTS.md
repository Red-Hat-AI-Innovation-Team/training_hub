...
- **Check vLLM's transitive dependency pins before setting version floors.** vLLM through v0.19.0 hard-pins `numba==0.61.2`. All three repos (mini_trainer, training_hub, instructlab/training) initially set `numba>=0.62.0`, which conflicted with vLLM's pin — requiring 3 coordinated PRs to relax the floor. When setting version floors for packages that are also transitive dependencies of vLLM, check vLLM's pins first: `pip show vllm | grep Requires` or read vLLM's `pyproject.toml`.
...
- **`kernels==0.15.x` and `trl<=0.24.0` incompatible with `transformers>=5.9.0`.** The `kernels` package constructs `LayerRepository` without the `revision`/`version` argument added in transformers 5.x, causing `TypeError` at model load. Fix: `pip uninstall kernels`. Also: `trl<=0.24.0` version-check import guards return tuples instead of bools under transformers v5. When upgrading transformers to v5, audit all installed packages that hook into the model loading pipeline and test in a fresh venv.
- **FSDP1 HYBRID_SHARD fails when `nproc_per_node` < total GPU count.** FSDP1 auto-detects `num_devices_per_node` (defaulting to 8 on multi-GPU nodes), then uses it to construct the device mesh. If you launch with fewer processes than GPUs (e.g., `torchrun --nproc_per_node=4` on an 8-GPU node), the auto-detected value of 8 exceeds `world_size=4` and FSDP raises `ValueError: group_size must not exceed world_size`. Fix: provide explicit `device_mesh` configuration instead of relying on auto-detection.
- **Gemma 3 models dual-register as text-only and VLM in HuggingFace transformers.** The model config class is registered under both config names, breaking VLM detection logic that assumes a model is either text-only or VLM but not both. If a new model fails VLM detection despite being a known VLM architecture, check whether it has dual-registered configs in transformers' auto-config mapping. String-based class name checks work as a fallback.
- **GEPA prompt optimization fails silently with small models.** Models smaller than ~7B cannot perform the meta-reasoning required for GEPA's reflection step. Additionally, trivial evaluation datasets (e.g., simple math) show no prompt optimization benefit because the base model already achieves high scores. Use models >=7B and datasets like GSM8K where prompting strategy matters.

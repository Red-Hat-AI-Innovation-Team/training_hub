...
- **Seed checkpoint placed before `model.register()` was wiped by ART:** The initial fix created a seed LoRA checkpoint at the expected path before calling `model.register(backend)`. This worked locally because our environment happened to not wipe the directory, but on RHOAI ART's registration recreated the project directory structure and deleted the seed. The fix had to be moved to a monkey-patch on `convert_checkpoint_if_needed`, which runs AFTER ART's directory setup + `save_model` but BEFORE vLLM's `add_lora`. Lesson: when hooking into a third-party library's lifecycle, read the full initialization flow (not just the entry point) to understand where directories are created and destroyed. A fix that works locally but fails in a different environment likely has a timing/ordering assumption that doesn't hold.

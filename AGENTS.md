...
- **Standalone and integrated installs can resolve dependencies differently.** mini_trainer installed standalone (without training_hub) can resolve to incompatible dependency versions that training_hub's resolver avoids. Example: unpinned `datasets` allows pyarrow 24.0.0 + datasets 2.14.4 standalone, which crashes all test collection because `PyExtensionType` was removed in pyarrow 21. Always test both standalone `pip install mini_trainer` and integrated `pip install training_hub` paths when changing dependency floors.
...
- **Both training backends now support TrainerCallback with 13 lifecycle hooks.** instructlab/training (PR #706) and mini_trainer (PR #109) added callback systems. Register callbacks via `TrainingArgs(callbacks=[...])`. Callbacks are serialized across the torchrun subprocess boundary via `inspect.getsource()` + base64 — all imports MUST be inside the callback method body, not at module level. Callbacks are async fire-and-forget with exception isolation (a crashing callback will not kill training). `on_train_end` blocks up to 10 seconds for cleanup callbacks. Available hooks: `on_train_begin`, `on_train_end`, `on_epoch_begin`, `on_epoch_end`, `on_step_begin`, `on_step_end`, `on_before_forward`, `on_after_backward`, `on_before_optimizer_step`, `on_log`, `on_evaluate`, `on_save`.
...
- **Narrow exception handlers in rollout/adapter code.** Catch only expected network errors (`ConnectionError`, `TimeoutError`) in rollout functions — broad `except Exception` masks CUDA OOM, `ImportError`, and configuration bugs as zero-reward rollouts that silently degrade training. Also ensure error-path return objects match happy-path types (e.g., use proper SDK `Choice` objects, not raw dicts) to avoid downstream type errors in the training loop.
...
- **Import unsloth before art in ART subprocess entry.** trl 1.x imports `GuidedDecodingParams` from vLLM, which does not exist in vLLM <= 0.21. Unsloth patches `vllm.sampling_params` with a shim, but only if imported first. The subprocess entry point (`_subprocess_entry`) must `import unsloth` before `import art`. If ART crashes with `ImportError: cannot import name 'GuidedDecodingParams' from 'vllm.sampling_params'`, check import order.
- **LoRA SFT packing config must be in SFTConfig, not SFTTrainer kwargs.** In trl 1.x, Unsloth auto-enables `padding_free=True` on `SFTConfig`. If `packing` is passed as a `SFTTrainer` constructor kwarg instead of in `SFTConfig`, trl validation raises `ValueError: When padding_free=True` because it sees padding_free without packing. Set `packing=True` and `packing_strategy='bfd'` in `SFTConfig` construction. This is backward-compatible with trl 0.24.0.
- **Both SFT backends default to AdamW beta2=0.95** (vs PyTorch's 0.999). This is intentional for training stability but may surprise users who assume PyTorch defaults. The four AdamW params (`beta1=0.9`, `beta2=0.95`, `eps=1e-8`, `weight_decay=0.0`) are first-class API params on `sft()` and `osft()` with a translation layer mapping beta1+beta2 to the `adamw_betas` tuple.
- **GEPA prompt optimization requires capable reflection models and budget calibration.** The `reflection_lm` must be able to reason about why prompts succeed or fail — models < 4B produce garbage reflections. `max_metric_calls` must be calibrated against dataset size: with N samples, each evaluation pass costs N calls, so `max_metric_calls / N` determines how many candidate prompts can be explored. Budget too low = no meaningful optimization.
- **Models with custom HuggingFace code require trust_remote_code=True everywhere.** Nemotron, Ministral, Qwen3.5, and other models with custom code need `trust_remote_code=True` in ALL `AutoConfig`, `AutoTokenizer`, and `AutoModel` calls. Without it, config loading fails with `KeyError` or architecture detection errors. Additionally, not all architectures support `gradient_checkpointing_enable()` — NemotronH (hybrid Mamba/MoE/Attention) raises `ValueError`. Wrap gradient checkpointing calls in try/except for `ValueError`, `NotImplementedError`, and `AttributeError`.
...
- **vLLM torch.fx crash on NVIDIA drivers 580+/610+:** vLLM 0.19-0.23 with torch 2.10-2.11 crashes with `RuntimeError: Tried to erase Node size_X but it still had N users in the graph` during torch.fx compilation. This is a driver-triggered compilation issue, not an inference bug. Fix: pass `enforce_eager=True` to `lora_grpo()` / `grpo()` to disable torch.compile and CUDA graph capture in vLLM. Lesson: when vLLM crashes during compilation (not inference), try `enforce_eager=True` before investigating driver/torch version downgrades.
- **assistant_only_loss=True applied to passthrough datasets:** LoRA SFT config set `assistant_only_loss=True` unconditionally, but for passthrough datasets (`dataset_type='passthrough'`, `dataset_text_field='text'`), this applies conversation-style loss masking to non-conversational data, silently degrading training. Fix: set `assistant_only_loss=False` when `dataset_type == 'passthrough'`. Lesson: boolean config defaults that assume conversational data will silently degrade non-conversational training quality without any error.
- **Gemma 3 SFT training crash from dual-registered VLM config:** `Gemma3Config` is registered in both `MODEL_FOR_CAUSAL_LM_MAPPING` and `MODEL_FOR_IMAGE_TEXT_TO_TEXT_MAPPING` in HuggingFace transformers. `AutoModelForCausalLM.from_pretrained` loads the full VLM (`Gemma3ForConditionalGeneration`) instead of text-only, crashing during FSDP training. Fix (instructlab/training #695): detect model classes with `ForConditionalGeneration` in the name and extract the text backbone. Lesson: when adding support for new model families, check whether their config is dual-registered as both CausalLM and VLM in transformers' auto mappings.

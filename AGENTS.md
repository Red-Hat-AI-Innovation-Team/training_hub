...
- **ART subprocess requires unconditional `os._exit()` to prevent vLLM 0.21+ hang.** vLLM V1's EngineCore and APIServer spawn non-daemon threads that prevent `asyncio.run()` from completing its shutdown. Without `os._exit()`, `proc.join()` in the parent hangs indefinitely after training completes. The fix uses `os._exit(0)` on success and `os._exit(1)` on failure in `_run_training`'s `finally` block — this MUST be unconditional (fire on both success and error paths). An earlier version only called `os._exit(0)` when `result is not None`, which skipped the force-exit on exception paths and re-introduced the hang. Results are saved to disk before exit, so the parent reads them via `training_results.json`/`.training_error` files after `proc.join()` returns.
- **Parent must use `proc.join(timeout=...)`, not blocking `proc.join()`.** Even with `os._exit()` in the subprocess, `asyncio.run()` can hang during its internal event loop teardown — after the coroutine's `finally` block completes but before `asyncio.run()` returns control. This makes subprocess-side fixes alone insufficient. The parent should always join with a timeout and terminate the subprocess if it's still alive after the timeout expires. Default: 7200s, configurable via `TRAINING_HUB_JOIN_TIMEOUT` env var.
...
- **Conditional `os._exit()` re-introduced subprocess hang on error paths (training_hub [#132](https://github.com/Red-Hat-AI-Innovation-Team/training_hub/pull/132)):** The initial hang fix only called `os._exit(0)` when `result is not None`. When `_run_training_loop` raised an exception, `result` stayed `None`, the force-exit was skipped, and `asyncio.run()` hung on vLLM's non-daemon threads — same symptom as the original bug but only on failure paths. Fix: make `os._exit()` unconditional in the `finally` block, using `os._exit(0 if result is not None else 1)` to preserve the exit code. Lesson: when adding a force-exit to work around a shutdown hang, it must fire on ALL exit paths. A conditional force-exit creates a silent regression that only manifests on the error path — the hardest path to test.
- **Parent `proc.join()` blocked forever despite subprocess `os._exit()` (training_hub [#133](https://github.com/Red-Hat-AI-Innovation-Team/training_hub/pull/133)):** Even with unconditional `os._exit()` in the subprocess, the parent's `proc.join()` could still hang indefinitely. Root cause: `asyncio.run()`'s internal event loop teardown (shutting down executors, closing transports) blocks on vLLM's non-daemon threads BEFORE returning control to the `finally` block where `os._exit()` lives. So the subprocess-side fix was necessary but not sufficient — the hang occurred at a deeper level than the coroutine. Fix: replace `proc.join()` with `proc.join(timeout=7200)` + `proc.terminate()` on the parent side. Results and checkpoints are already saved to disk before shutdown begins, so terminating is safe. Lesson: defense-in-depth — subprocess force-exit fixes the common case, parent-side timeout fixes the edge case where the force-exit itself is unreachable.
...
# Validated AIPCC 3.5 GA stack (ART backend, training_hub PRs #127-133)
# Use this stack for GRPO training on RHOAI AIPCC 3.5 GA
torch==2.11.0, openpipe-art==0.5.18, vllm==0.21.0+rhaiv.10
unsloth==2026.6.9+rhaiv.2, unsloth-zoo==2026.6.7+rhaiv.1
trl==1.8.0, transformers==5.13.0, kernels==0.16.0
Requires: megatron stub (auto-installed), no-op reward_funcs patch (auto-applied)
Verified: GRPO e2e on A100, checkpoint output, clean subprocess exit
```

...
- **Models with custom code require `trust_remote_code=True` in all auto-class calls.** Models like Nemotron, Ministral, and Qwen3.5 have custom modeling code in their HuggingFace repos. All `AutoConfig.from_pretrained()`, `AutoModelForCausalLM.from_pretrained()`, and `AutoTokenizer.from_pretrained()` calls must include `trust_remote_code=True`, or config loading fails with `KeyError` (e.g., on `hybrid_override_pattern`) or architecture detection errors. Missing this flag in even one location (e.g., tokenizer loading in data preprocessing) causes failures. (Ref: instructlab/training PR #696)
- **Gradient checkpointing is not universally supported.** Hybrid architecture models (e.g., NemotronH with Mamba/MoE/Attention layers) raise `ValueError` when `model.gradient_checkpointing_enable()` is called. Code that enables gradient checkpointing must wrap the call in try/except (catching `ValueError`, `NotImplementedError`, and `AttributeError`) and log a warning on failure rather than crashing. (Ref: instructlab/training PR #696)
- **Checkpoint auto-resume crashes on non-numeric `step_*` directories.** Auto-detection of full-state checkpoints scans `full_state_checkpoints/step_*` and assumes the suffix after `step_` is numeric. Directories like `step_latest` cause `ValueError`. If you encounter a `ValueError` during OSFT checkpoint auto-resume, check for non-numeric `step_*` directories. Filter defensively: only parse directories whose suffix after `step_` is all digits. (Ref: mini_trainer PR #96 review)
...
- **Gate console logging on LOCAL_RANK, not global rank.** Logging gated on `dist.get_rank() == 0` (global rank) means only node 0, GPU 0 prints training progress — worker nodes show zero console output (no loss, no lr, no tokens/sec, no tqdm). Switch to `int(os.environ.get("LOCAL_RANK", 0)) == 0` so each node's local rank 0 prints progress. Keep wandb/mlflow logging gated on global rank 0 to avoid duplicate metric entries. If you see missing console output on worker nodes during multi-node training, check whether logging is gated on global rank vs LOCAL_RANK. (Ref: mini_trainer PR #82)
...
- **Test new algorithms with configurations complex enough to exercise their distinguishing features.** When validating a new algorithm end-to-end, the test configuration must be complex enough to exercise the algorithm's core differentiating mechanism, or you get false positives ("optimization completed successfully" with no actual optimization). For GEPA, initial testing used a model too small for reflection, a budget too low to show improvement, and questions too easy to produce score variance. This generalizes the existing GRPO "make questions hard enough" guidance: for any algorithm, identify its distinguishing mechanism and ensure the test exercises it. Unit test passage is necessary but not sufficient — always attempt e2e testing. For training algorithms, run at least one real training loop. For notebooks, execute them end-to-end. For inference-dependent features (GEPA, GRPO), start the required inference server and run a real request. (Ref: training_hub PR #77, Multica MT-20/MT-21/MT-24)
- **Notebook and example defaults must be runnable on the stated minimum hardware.** When creating templated notebooks or examples, the default configuration (model + GPU count + algorithm parameters) must actually work without modification on the minimum hardware the notebook claims to support. An OSFT notebook defaulting to Llama-3.1-8B with `num_gpus=1` would OOM because OSFT's memory overhead makes this infeasible on a single GPU. Test the default configuration on the minimum hardware before merging — users try defaults first, and a notebook that fails out of the box erodes trust. (Ref: training_hub PR #75, Multica MT-24)
- **Before creating a PR, search for existing open PRs that address the same issue.** Run `gh pr list --state open --head <branch-pattern>` or search by title/label. If an open PR already addresses the same GitHub issue, update it instead of creating a new one. Duplicate PRs waste reviewer time and create merge conflicts. (Ref: Multica MT-14, where 8+ duplicate PRs were created for the same fix)
...
- **Backends that modify `os.environ` must use try/finally cleanup.** When a backend sets environment variables (e.g., `OPENAI_API_BASE`, `OPENAI_API_KEY` for LLM API calls), it must save the original values and restore them in a `finally` block. Without cleanup, env vars leak between sequential algorithm runs in the same process. Unlike ART (which runs in a spawned subprocess where env pollution is isolated), in-process backends share the parent's environment. A second algorithm call or any subsequent run inherits stale API configuration from the first. (Ref: training_hub PR #77)
- **EOS token ID 0 is valid — use `is not None` checks.** Some models use EOS token ID 0. Python truthiness checks like `if eos_id:` or `if not eos_id:` incorrectly treat 0 as falsy, skipping valid EOS handling. Always use `if eos_id is not None:` when checking for the presence of special token IDs. (Ref: instructlab/training PR #693)
- **transformers v5 renamed `additional_special_tokens` to `extra_special_tokens`.** Code that accesses special tokens must handle both names to maintain compatibility with transformers v4.57 (required by unsloth for the LoRA training path) and v5+ (used by the SFT/OSFT paths via instructlab-training). Use a version-conditional pattern or try/except when accessing these attributes. (Ref: instructlab/training PR #684)
- **Split heavy optional dependencies into separate extras groups.** When an algorithm has multiple backends where one requires a heavy dependency (e.g., `mlflow>=3.5.0` at ~200MB+), create separate extras groups rather than bundling everything into one. Example: `[gepa]` (lightweight, native backend only) and `[gepa-mlflow]` (adds mlflow). Each backend's heavy dependencies should be independently installable. (Ref: training_hub PR #77)
- **Never include internal infrastructure details in public GitHub comments.** Internal hostnames (e.g., `rh-a100-1`), IP addresses, infrastructure names, and internal environment specifics must not appear in GitHub PR comments or reviews. These are public-facing. Keep infrastructure details in Multica issue comments only. Use generic terms like "GPU server" instead of specific internal hostnames.
...
- **Duplicate PR flood for train_dtype fix:** The dev agent created 8+ PRs (#99-#107) for the same GitHub issue because each Multica issue triggered a fresh run that did not check for existing open PRs. Root cause: no pre-flight check for existing PRs. Fix: agents must run `gh pr list --state open --head <branch-pattern>` before creating any PR. If one exists, update it instead of creating a new one. Lesson: always search for existing PRs addressing the same GitHub issue before opening a new one.
- **Agent workspace Python/GPU mismatch:** Agents ran tests on the Multica workspace (Python 3.9, no GPU) and claimed success, but the project requires Python 3.11+ and GPU for meaningful validation. Tests that passed on CPU-only workspace failed on A100 due to missing CUDA paths. Lesson: always SSH to A100 for testing. Never claim tests passed if they only ran on the CPU-only workspace.

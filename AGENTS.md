...
### Working With GEPA (Prompt Optimization)

- **litellm requires `OPENAI_API_KEY` even for local endpoints.** When using `api_base` to point at a local vLLM server, litellm still validates that `OPENAI_API_KEY` is set. The GEPA backends set a dummy value (`"dummy"`) automatically when `api_base` is provided, but if you're calling litellm directly, remember to set it.
- **Reflection model needs sufficient capacity (3B+).** With 1.5B models, GEPA's reflection step produces garbage — JSON task descriptions or structured objects instead of improved system prompts. The model doesn't have enough capacity to understand the reflection prompt template. Use 3B+ for both `task_lm` and `reflection_lm`, or use a larger model specifically for reflection.
- **Example data must be hard enough to show prompt improvement.** If the model scores 80-90% with a vague initial prompt, GEPA has no room to improve and the notebook appears broken. Use questions the model genuinely struggles with — multi-step math, format-sensitive outputs, or domain-specific tasks where prompt wording measurably affects accuracy. Target ~20-40% baseline accuracy for a compelling demo. Budget formula: `num_samples * 20` gives ~20 iterations of optimization.
- **`reflection_minibatch_size` controls how many examples the reflection step sees.** Default may be too low for the reflection model to identify patterns in failures. Set `reflection_minibatch_size=5` to give the reflection step more context.

### Kubeflow / RHOAI Integration

- **Emit `max_steps` in training metrics for progress reporting.** The Kubeflow SDK metrics handler calculates `progressPercentage` from `max_steps`. Without it, GRPO jobs report progress as "unknown" (`None`). Set `max_steps` to `num_iterations` in both rollout and train phase entries of `training_metrics.jsonl`.
...
- **Pin litellm upper bound at `<1.82.7` (TeamPCP supply chain attack).** litellm versions 1.82.7–1.82.8 were compromised in March 2026. The `[grpo]` and `[gepa]` extras retain this cap via `!=1.82.7,!=1.82.8`. When updating the litellm floor, always preserve the exclusion. `openpipe-art` itself only requires `litellm>=1.71.1`, so the floor can be relaxed without security risk.

### Example Notebook Quality

- **Notebooks must demonstrate the feature's value, not just call its API.** A notebook where the optimization runs but doesn't improve (because the baseline is already high) is worse than no notebook — it suggests the feature doesn't work. Test the notebook end-to-end and verify the output tells a compelling story: clear baseline → measurable improvement → result inspection.
- **Test notebooks iteratively on real infrastructure.** The GEPA notebook required 6+ revision cycles because issues only surfaced during end-to-end runs (wrong function signatures, missing API keys, attribute errors on result objects, models too small for reflection). Each notebook cell should be run and verified, not just written.
...
- **AdamW optimizer destroyed OSFT orthogonality.** AdamW's element-wise moment rescaling (`m̂_t / (√v̂_t + ε)`) breaks the orthogonal structure of projected gradients, causing parameter updates to leak into the frozen subspace over training steps. The drift is invisible without explicit orthogonality checks. Fix: add post-step parameter re-projection that guarantees `U_low` and `V_low` never drift into the frozen subspace. Lesson: when using constrained optimization (orthogonal subspace, manifold optimization), always verify that the optimizer preserves the constraint — standard optimizers like AdamW operate element-wise and will destroy geometric structure.
- **GEPA notebook required 6+ fix iterations.** The initial notebook used trivia questions that the model answered correctly 80-90% of the time, leaving no room for prompt optimization to improve. Then the reflection model (1.5B) produced garbage proposals. Then `predict_fn` had the wrong signature for the MLflow backend. Then `GEPAResult` didn't have the expected attributes. Then `mlflow.genai.get_prompt` didn't exist in the installed version. Each issue was only discoverable by running the notebook end-to-end. Lesson: example notebooks need the same level of functional testing as production code.
- **OSFT `train_dtype` not forwarded to model loading.** `_load_model_memory_efficient` in mini_trainer used a hardcoded dtype instead of the user's `train_dtype` parameter. Models loaded in the wrong precision, causing silent accuracy differences. Fix: pass `train_dtype` explicitly. Lesson: when a function accepts a dtype/precision parameter, trace it through every code path that loads or initializes model weights.

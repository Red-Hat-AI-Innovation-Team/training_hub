...
- **Checkpoint auto-resume crashes on non-numeric `step_*` directories.** Auto-detection of full-state checkpoints scans `full_state_checkpoints/step_*` and assumes the suffix after `step_` is numeric. Directories like `step_latest` cause `ValueError`. Filter defensively: only parse directories whose suffix after `step_` is all digits. (Ref: mini_trainer PR #96 review)
...
- **Gate console logging on LOCAL_RANK, not global rank.** Logging gated on `dist.get_rank() == 0` (global rank) means only node 0, GPU 0 prints training progress — worker nodes show zero console output (no loss, no lr, no tokens/sec, no tqdm). Switch to `int(os.environ.get("LOCAL_RANK", 0)) == 0` so each node's local rank 0 prints progress. Keep wandb/mlflow logging gated on global rank 0 to avoid duplicate metric entries. (Ref: mini_trainer PR #82)
...
- **Test new algorithms with configurations complex enough to exercise their distinguishing features.** When validating a new algorithm end-to-end, the test configuration must be complex enough to exercise the algorithm's core differentiating mechanism, or you get false positives ("optimization completed successfully" with no actual optimization). For GEPA, initial testing used a model too small for reflection, a budget too low to show improvement, and questions too easy to produce score variance. For any algorithm, identify its distinguishing mechanism and ensure the test exercises it. Unit test passage is necessary but not sufficient — always attempt e2e testing. (Ref: training_hub PR #77, Multica MT-20/MT-21/MT-24)
- **Notebook and example defaults must be runnable on the stated minimum hardware.** When creating templated notebooks or examples, the default configuration (model + GPU count + algorithm parameters) must actually work without modification on the minimum hardware the notebook claims to support. Test the default configuration on the minimum hardware before merging — users try defaults first, and a notebook that fails out of the box erodes trust. (Ref: training_hub PR #75, Multica MT-24)
- **Before creating a PR, search for existing open PRs that address the same issue.** Run `gh pr list --state open --head <branch-pattern>` or search by title/label. If an open PR already addresses the same GitHub issue, update it instead of creating a new one. Duplicate PRs waste reviewer time and create merge conflicts. (Ref: Multica MT-14)
...
- **Backends that modify `os.environ` must use try/finally cleanup.** When a backend sets environment variables (e.g., `OPENAI_API_BASE`, `OPENAI_API_KEY` for LLM API calls), it must save the original values and restore them in a `finally` block. Without cleanup, env vars leak between sequential algorithm runs in the same process. Unlike ART (which runs in a spawned subprocess where env pollution is isolated), in-process backends share the parent's environment. (Ref: training_hub PR #77)

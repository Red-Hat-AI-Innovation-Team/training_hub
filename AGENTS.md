...
- **ART subprocess must `import unsloth` before `import art`.** The ART subprocess imports `art.unsloth.service`, which pulls in TRL, which tries to import `GuidedDecodingParams` from vLLM. This class doesn't exist in vLLM ≤0.21, causing `ImportError`. Unsloth patches `vllm.sampling_params` with a shim, but only if imported first. The `_subprocess_entry` function handles this — do not reorder or remove the early `import unsloth`.
- **Use `enforce_eager=True` on nodes with newer NVIDIA drivers (580+/610+).** vLLM's torch.fx compilation pipeline crashes with `RuntimeError: Tried to erase Node size_X but it still had N users in the graph` on vLLM 0.19–0.23 with torch 2.10–2.11 and newer drivers. Setting `enforce_eager=True` disables torch.compile and CUDA graph capture, bypassing the issue. The parameter is exposed on `lora_grpo()`, `grpo()`, and `LoRAGRPOAlgorithm.train()`.
...
- **vLLM torch.compile crash on NVIDIA driver 580+/610+:** ART GRPO crashed with `RuntimeError: Tried to erase Node size_X but it still had N users in the graph` in vLLM's torch.fx compilation pipeline. Affected vLLM 0.19–0.23 with torch 2.10–2.11 on nodes with newer drivers. Fix: pass `enforce_eager=True` to disable torch.compile and CUDA graph capture. Lesson: vLLM's compilation pipeline is sensitive to driver versions — when you see torch.fx graph errors, try `enforce_eager=True` before downgrading packages.
- **TRL `GuidedDecodingParams` import failure in ART subprocess:** The ART subprocess crashed with `ImportError` because TRL tried to import `GuidedDecodingParams` from vLLM (doesn't exist in vLLM ≤0.21). Unsloth patches this class into `vllm.sampling_params`, but only if imported first. Fix: add `import unsloth` before `import art` in `_subprocess_entry`. Lesson: import order matters when libraries monkey-patch third-party modules — the patcher must be imported before any consumer of the patched API.

...
│   ├── lora.py              # LoRA + SFT
│   ├── gepa.py              # GEPA prompt optimization (gradient-free, evolutionary)
...
- **Import Unsloth before ART in spawned subprocesses.** TRL (imported transitively by `art.unsloth.service`) tries to import `GuidedDecodingParams` from vLLM, which doesn't exist in vLLM ≤0.21. Unsloth patches `vllm.sampling_params` with a shim, but only if imported first. In `_subprocess_entry`, add `import unsloth` (wrapped in try/except ImportError) before `import art`. Without this, the subprocess crashes with `ImportError: cannot import name 'GuidedDecodingParams'`.
- **Use `enforce_eager=True` on nodes with NVIDIA driver 580+/610+.** vLLM 0.19-0.23 with torch 2.10-2.11 crashes during torch.fx compilation with `RuntimeError: Tried to erase Node size_X but it still had N users in the graph`. This is a torch.compile bug triggered by newer CUDA drivers. Setting `enforce_eager=True` disables torch.compile and CUDA graph capture in vLLM, bypassing the issue entirely. The parameter is exposed on `lora_grpo()`, `grpo()`, and `LoRAGRPOAlgorithm.train()`.
...
- **torch.compile crash on NVIDIA driver 580+/610+:** ART backend crashed in the vLLM subprocess with `RuntimeError: Tried to erase Node size_X but it still had N users in the graph` during torch.fx compilation. This only manifested on nodes with newer NVIDIA drivers and affected vLLM 0.19-0.23 with torch 2.10-2.11. The root cause was a torch.compile bug interacting with the newer driver's CUDA runtime. Fix: expose `enforce_eager=True` parameter to disable torch.compile and CUDA graph capture in vLLM. Additionally, TRL's import of `GuidedDecodingParams` from vLLM (which doesn't exist in vLLM ≤0.21) required importing Unsloth first in the subprocess to apply its compatibility shim. Lesson: driver upgrades can break torch.compile pipelines even when the CUDA toolkit version is unchanged — when vLLM crashes during compilation, `enforce_eager` is the first thing to try.

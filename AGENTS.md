...
- **`grpo()` is a thin wrapper around `lora_grpo()` with `lora_r=0` — the abstraction is inverted.** `grpo()` delegates to `lora_grpo()` instead of the reverse. This is acknowledged tech debt; if you refactor this area, consider inverting the dependency so `lora_grpo()` calls `grpo()` with LoRA extensions. For now, changes to the GRPO path must account for this — `grpo()`-specific kwargs should be validated to reject LoRA-only parameters that would be silently forwarded.
- **Kubeflow progress reporting requires `max_steps` in `training_metrics.jsonl`.** The Kubeflow SDK metrics handler calculates `progressPercentage` from `max_steps`. For GRPO, `max_steps` is set to `num_iterations`. Without it, the SDK reports progress as `None`. If you add a new algorithm, include `max_steps` in both rollout and train phase metric entries.
...
### OSFT-Specific (mini_trainer)

- **AdamW breaks orthogonal structure — post-step re-projection is required.** Adaptive optimizers (AdamW, Adam) apply element-wise moment rescaling (`m̂_t / (√v̂_t + ε)`) that destroys the orthogonal structure of projected gradients, causing parameter updates to leak into the frozen subspace over time. mini_trainer now applies `project_parameters()` after each optimizer step. If you add a new optimizer or modify the training loop, ensure the post-step re-projection is preserved.
- **Factored V-projection is 2-7x faster than the Gram matrix form under FSDP2.** The original `dV -= dV @ (V_high^T @ V_high)` required an `(M, M)` all-reduce. The factored form `dV -= (dV @ V_high^T) @ V_high` replaces this with a `(k_high, M)` all-gather. Set `OSFT_CACHE_V=1` env var to cache the frozen V_high tensor for additional speedup (opt-in because it uses extra memory).
- **OSFT supports on-demand full-state checkpointing for Kubernetes/SLURM.** Signal-driven (SIGTERM, SIGUSR1, etc.) checkpointing with distributed consensus via `all_reduce(MAX)`. Workers poll a trigger file at batch boundaries. The trigger file is now named `checkpoint_requested` (renamed from `mini_trainer_checkpoint_trigger`). Override with `CHECKPOINT_TRIGGER_FILENAME` env var if needed.
- **OSFT auto-resumes from the latest full-state checkpoint.** When `on_demand_checkpointing=True` and no explicit `resume_from_full_state_checkpoint` is given, `find_latest_full_state_checkpoint()` scans `{output_dir}/full_state_checkpoints/` for the latest valid `step_*` directory containing `training_state.pt`.

### Past Incidents (continued)

- **ART async cleanup caused false-positive training failures on Kubeflow.** After ART training completes, the background health-check task throws `ConnectionRefusedError` because vLLM has shut down. This caused the subprocess to write to `error_path` even though training succeeded, making Kubeflow restart the pod. Fix: check if `results_path` exists before treating a cleanup exception as a failure. Lesson: async background tasks (health checks, heartbeats) that outlive the main operation must distinguish between "service failed" and "service finished successfully."
- **`json.loads()` on tool_call arguments can return non-dict types.** When deserializing tool call arguments from JSON strings, `json.loads()` can return `[]`, `"x"`, or `null` — not just dicts. If the Qwen3.5 chat template then calls `.items()` on the result, it fails with the same error the deserialization was meant to prevent. Fix: validate that the decoded value is a `dict` and fall back to `{}` otherwise.

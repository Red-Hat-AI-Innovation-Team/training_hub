...
- **Newer NVIDIA drivers (580+/610+) crash vLLM's torch.compile pipeline.** vLLM 0.19-0.23 with torch 2.10-2.11 fails with `RuntimeError: Tried to erase Node size_X but it still had N users in the graph` during torch.fx compilation. Pass `enforce_eager=True` to `lora_grpo()` or `grpo()` to disable torch.compile and CUDA graph capture, bypassing the issue.
- **Unsloth must be imported before ART in the subprocess.** ART's subprocess imports trl's `GRPOTrainer`, which tries to import `GuidedDecodingParams` from vLLM — this class doesn't exist in vLLM ≤0.21. Unsloth patches the shim, but only if imported first. The fix is already applied in `_subprocess_entry` — if you modify subprocess initialization, preserve the `import unsloth` before `import art` order.
...
- **vLLM torch.compile crash on newer NVIDIA drivers (580+/610+):** vLLM 0.19-0.23 with torch 2.10-2.11 crashed with `RuntimeError: Tried to erase Node size_X but it still had N users in the graph` during torch.fx compilation on nodes with newer NVIDIA drivers. Not a vLLM bug per se — the torch.fx compilation pipeline has a driver-version-dependent code path. Fix: pass `enforce_eager=True` to disable torch.compile and CUDA graph capture. The parameter was added to `lora_grpo()`, `grpo()`, and `LoRAGRPOAlgorithm.train()` in training_hub PR #98. Lesson: when a crash appears driver-version-specific and involves compiled GPU code, try disabling compilation first.
- **trl `GuidedDecodingParams` import failure in ART subprocess:** trl tries to import `GuidedDecodingParams` from vLLM at import time, but this class doesn't exist in vLLM ≤0.21. Unsloth patches in a shim, but only if `import unsloth` runs first. In the ART subprocess, `import art` ran before unsloth, so the shim wasn't in place. Fix: add `import unsloth` before `import art` in `_subprocess_entry`. Lesson: import order matters when libraries patch third-party modules — document and protect the order if your subprocess re-imports from scratch.
...
- **`osft_output_dtype` was removed.** OSFT output dtype now follows `train_dtype` directly (this was already the fallback behavior when `osft_output_dtype` was `None`). Users control precision via three remaining parameters: `osft_upcast_dtype` (computation precision, default float32), `train_dtype` (training precision), and `save_dtype` (checkpoint precision). See mini_trainer PR #108.

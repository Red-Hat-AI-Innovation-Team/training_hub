...
│   ├── gepa.py              # GEPA gradient-free prompt optimization
...
- **`datasets` and `pyarrow` versions must be compatible.** `pyarrow>=21.0.0` removed `PyExtensionType`, but `datasets<5.0.0` still uses it. An unpinned `datasets` dependency lets pip resolve `datasets==2.x` alongside `pyarrow==24.x`, which crashes at import/test-collection time with `AttributeError: module 'pyarrow' has no attribute 'PyExtensionType'`. Pin `datasets>=5.0.0` when using modern pyarrow. This can be masked in development environments where training_hub's own dependency tree pulls in the correct datasets version — only surfaces in standalone installs.
...
- **LoRA SFT `packing` must be set in `SFTConfig`, not as `SFTTrainer` kwargs.** In trl 1.x, Unsloth auto-enables `padding_free=True` on `SFTConfig`. When `packing` is passed only as a `SFTTrainer` constructor kwarg (not in `SFTConfig`), trl's validation sees `padding_free=True` without packing and raises `ValueError: When padding_free=True without packing, max_length is not enforced`. The fix is to set `packing=True` and `packing_strategy="bfd"` directly in the `SFTConfig` constructor and remove `packing` from the `SFTTrainer` kwargs. This is backward-compatible with trl 0.24.0 (SFTConfig already accepts these parameters). Also check `assistant_only_loss`: for non-conversational datasets (`dataset_text_field="text"`), it must be `False` — trl rejects `assistant_only_loss=True` on non-message datasets.
...
- **`datasets`/`pyarrow` version mismatch broke standalone mini_trainer installs:** An unpinned `datasets` dependency allowed pip to resolve `datasets==2.14.4` alongside `pyarrow==24.0.0`. `pyarrow>=21.0.0` removed `PyExtensionType`, but `datasets<5.0.0` still references it. All test collection failed with `AttributeError: module 'pyarrow' has no attribute 'PyExtensionType'`. This was invisible in development because training_hub's dependency tree already pulled in the correct datasets version. Fix: pin `datasets>=5.0.0` in mini_trainer's `pyproject.toml`. Lesson: test dependency resolution in standalone installs, not just in the full training_hub development environment — transitive resolution can mask incompatible version combinations.
- **trl 1.x `padding_free` crash with SFTTrainer packing kwarg:** After upgrading to trl 1.x, LoRA SFT training crashed with `ValueError: When padding_free=True without packing, max_length is not enforced`. Unsloth auto-enables `padding_free=True` on `SFTConfig`, but `packing` was only passed as a `SFTTrainer` constructor kwarg (not in `SFTConfig`). trl's validation saw `padding_free=True` without `packing=True` in the config and rejected it. Fix: move `packing` and `packing_strategy` into `SFTConfig` construction and remove them from `SFTTrainer` kwargs. This is backward-compatible with trl 0.24.0. Lesson: when upgrading training frameworks, check which config object each parameter is read from — parameters may move between config objects across versions, and auto-config features (like Unsloth's `padding_free`) can trigger validation that was previously bypassed.

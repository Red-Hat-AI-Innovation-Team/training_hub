...
- **Both SFT and OSFT backends default to `beta2=0.95` for AdamW, not PyTorch's `0.999`.** This non-standard default is set in both `instructlab.training.TrainingArgs` and `mini_trainer.TrainingArgs`. It causes more aggressive decay of the squared-gradient moving average, which can affect training stability. Users comparing runs against standard AdamW configurations or porting hyperparameters from other frameworks should be aware of this difference. The `beta1`, `beta2`, `eps`, and `weight_decay` parameters are all exposed as first-class kwargs on `sft()` and `osft()`.

...
- **ValidationScheduler triggers must be coalesced per step, with per-type state tracking.** When multiple validation triggers (step, epoch, samples, final) fire on the same training step, validation must run at most once — without coalescing, `compute_validation_loss()` runs twice and the second `batch_metrics.update()` silently overwrites the first. Additionally, each trigger type needs its own last-validated sample counter; a shared counter causes step-based validation to suppress epoch-end and final triggers because `accumulated_samples > last_validated_samples` fails when they're equal. This was a P0 caught in review of mini_trainer PR #116.

#!/usr/bin/env python3
"""
OSFT Training Example: Ministral 3 3B on Medical Flashcards Dataset

This script demonstrates OSFT (Orthogonal Subspace Fine-Tuning) of the Ministral 3 3B
...
Mistral AI. OSFT targets the standard self_attn projections (q, k, v, o) and MLP
projections across all layers. Note that Liger kernels are not yet supported for
the Ministral 3 architecture, so use_liger is set to False.
...
Example usage:
    python osft_ministral_medical_example.py \\
...
def main():
    parser = argparse.ArgumentParser(
        description="OSFT Training Example: Ministral 3 3B on Medical Flashcards Dataset"
    )
    ...
    print("OSFT Training: Ministral 3 3B on Medical Flashcards Dataset")
    print("=" * 60)
    ...
    try:
        osft(
            ...
            warmup_steps=0,
            # Optimization (Liger not supported for Ministral 3 architecture)
            use_liger=False,
            seed=42,
            lr_scheduler="cosine",
            # Checkpointing
